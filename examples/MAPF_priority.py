...
class PriorityMAPFApp(Application): #inherits from Application -> Clingo's base class for defining applications
    ...
    def __init__(self):
        self._delta : Optional[int]      = None # optional delta/horizon value
        self._reach :Flag                           = Flag(True) # whether to compute reach atoms via cmapf or ASP
        self._summary :Flag                         = Flag(True) # whether to add per agent costs to models
    ...
    def register_options(self, options: ApplicationOptions) -> None: # to add custom options to a clingo based application, called during initialization
        ...
        options.add_flag(
            "PriorityMAPF", "show-costs", "add per agents costs to model", self._summary
        )
    ...
    def _prepare(self, ctl: Control, problem: Problem) -> Optional[Sequence[Tuple[str, Sequence[Symbol]]]]:
        ...
        if self._delta is None:
            ...
            start = timeit.default_timer()
            delta = problem.min_delta(self._objective) #always 1?
        ...
        if self._objective == Objective.MAKESPAN:
            parts+=([("makespan", [Number(delta)]), ("makespan", [])]) #program makespan(horizon).
            self._stats["Horizon"] = delta
        else:
            parts+=([("sum_of_costs", [Number(delta)]), ("sum_of_costs", [])]) #program sum_of_costs(delta).
        ...
        if self._reach:
            ...
            start = timeit.default_timer()
            if not problem.add_reachable(ctl, self._objective, delta): #sp_length is calculated within compute_reach
                parts = None
